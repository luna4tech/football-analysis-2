...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None,
    ...
    # Drawing sizes. When left as None these reproduce the original look for
    # callers that don't override them (e.g. demo.py); render_from_txt passes
    # smaller values so dense, wide-angle footage stays readable.
    ...
    header_scale = text_scale if text_scale > 1.0 else 1.0
    cv2.putText(im, 'frame: %d fps: %.2f num: %d' % (frame_id, fps, len(tlwhs)),
                (0, int(15 * header_scale)), cv2.FONT_HERSHEY_PLAIN, header_scale, (0, 0, 255), thickness=2)
